...
# grad_reg = GradientBoostingRegressor(alpha=0.9, criterion='friedman_mse', init=None,
#                                      learning_rate=0.08, loss='ls', max_depth=6, max_features=0.8,
...
# scores = cross_val_score(grad_reg, X, y, scoring="neg_mean_squared_error", cv=5)
# rmse_scores = np.sqrt(-scores)
# print(np.mean(rmse_scores).round(2))
# print(np.std(rmse_scores).round(2))
# print(np.log(np.mean(rmse_scores)).round(3))

data_matrix = xgb.DMatrix(data=X_train, label=y_train)

xgb_reg = xgb.XGBRegressor(n_estimators=300, learning_rate=0.08, reg_alpha=0.9, max_depth=6, subsample=0.9)
...
for pred, actual, diff, perc in zip(preds, y_test, diffs, percs):
    # f.write(str(np.round(pred, decimals=0)) + ',' + str(actual) + ',' + str(np.round(pred - actual, decimals=0)) + ',' + str(np.round(pred / actual, decimals=2)) + '\n')
